...
class DataSceneProcessor:
    ...
    def process_scene(self, scene_data, scene_id):
        ...
        for point in room_points:
            ...
            classify_using_image = False # use image only when object list is inadequate
            ...
            elif (objs_at_this_pos.issubset(self.common_objs)):
                print("Only common objects -- not skipping this time, but classifying with image only")
                classify_using_object_list = False
                classify_using_image = True
            elif (len(objs_at_this_pos) < 3):
                print("Very few objects -- not skipping, but classifying with both image and object set")
                classify_using_object_list = True
                classify_using_image = True
            else:
                classify_using_object_list = True
                classify_using_image = False
